...
-   try shrinking corners to the intersection of the two inset lines parallel to their edges.
-   instead of unshrinking only the nearest, try all within a threshold of mouse
-   grid of squares with one split only, with random significant shrinkage. looks bold.
-   quads unshrink by colour, periodically? (or all quads with same colour as hovered-quad?)
-   unshrink all quads on one or two perpendicular edges (or intersecting one or two simple straight lines across the design. Really wants to be a thick line though to ensure fewer near misses)
    this would look good animated (stagger might work but should be in order they line passes through them)
-   animate a little humanised rotation around centroid?
